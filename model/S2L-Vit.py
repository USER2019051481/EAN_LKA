...
from model.lka import MyDecoderLayer
from model.SCTB import ChannelTransformer, get_CTranS_config
...
class S2LVit(nn.Module):
    ...
    def __init__(self,
        ...
        self.SCT = ChannelTransformer(get_CTranS_config(), False, 256,
                                       channel_num=[96, 192, 384, 768],
                                       patchSize=get_CTranS_config().patch_sizes)
    ...
    def forward(self, x):
        ...
        x1, x2, x3, x4, att_weights = self.SCT(feature[0], feature[1], feature[2], x)
        ...
        tmp_3 = self.decoder_3(x4.permute(0, 2, 3, 1).view(b, -1, c))
        tmp_2 = self.decoder_2(tmp_3, x3.permute(0, 2, 3, 1))
        tmp_1 = self.decoder_1(tmp_2, x2.permute(0, 2, 3, 1))
        tmp_0 = self.decoder_0(tmp_1, x1.permute(0, 2, 3, 1))
        return tmp_0
...
if __name__ == "__main__":
    ...
    net = S2LVit(embed_dim=96, depths=(10, 18, 36, 10), mlp_ratio=2, drop_path_rate=0.4, n_div=12)
    # net = ShiftViT(embed_dim=128, depths=(10, 18, 36, 10), mlp_ratio=2, drop_path_rate=0.6, n_div=16).cuda(0)
    # net.load_state_dict(torch.load("../checkpoint/shiftvit_small_r2.pth"), strict=False)
    output = net(input)
